...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel, long when, ConnectionStatus status) {
        ...
        android.app.Notification.Builder nbuilder = null;
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
		{
			NotificationChannel channelx = new NotificationChannel(CHANNEL_ID, CHANNEL_NAME, NotificationManager.IMPORTANCE_LOW);
			channelx.setDescription(CHANNEL_NAME);
			channelx.enableLights(true);
			channelx.setLightColor(Color.BLUE);
			mNotificationManager.createNotificationChannel(channelx);
        ...
        if (mProfile != null) {
           ...
           // nbuilder.setContentTitle(getString(R.string.notifcation_title, /*mProfile.mConnections[0].mServerName, mProfile.mUsername*/ getApplicationContext().getString(R.string.app_name), ApplicationBase.getSharedPreferences().getString("SSHName", "")));
        } else {
