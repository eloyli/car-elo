...
class LogFileHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
        try {
            if (msg.what == LOG_INIT) {
                ...
                trimLogFile();
                for (LogItem li : VPNLog.getlogbuffer())
            ...
            e.printStackTrace();
            VPNLog.logError("Error during log cache: " + msg.what);
            VPNLog.logException(e);
        }
    }
    ...
    private void readLogCache(File cacheDir) {
        try {
            ...
            VPNLog.logError("Reading cached logfile failed");
            VPNLog.logException(e);
            e.printStackTrace();
            // ignore reading file error
        } finally {
            synchronized (VPNLog.readFileLock) {
                VPNLog.readFileLog = true;
                VPNLog.readFileLock.notifyAll();
            }
        }
    }
        ...
        while (read >= 5) {
            ...
            while (buf[skipped] != MAGIC_BYTE) {
                ...
                if (!(logFile.read(buf, skipped + 4, 1) == 1) || skipped + 10 > buf.length) {
                    VPNLog.logDebug(String.format(Locale.US, "Skipped %d bytes and no a magic byte found", skipped));
                    break readloop;
                }
            }
            if (skipped > 0)
                VPNLog.logDebug(String.format(Locale.US, "Skipped %d bytes before finding a magic byte", skipped));
            ...
            while (pos < len) {
                ...
                if (b == MAGIC_BYTE) {
                    VPNLog.logDebug(String.format(Locale.US, "Unexpected magic byte found at pos %d, abort current log item", pos));
                    ...
                    if (b == 0)
                        ...
                        VPNLog.logDebug(String.format(Locale.US, "Escaped byte not 0 or 1: %d", b));
                        read = logFile.read(buf, 1, 4) + 1;
                        continue readloop;
                    }
                }
            ...
            itemsRead++;
            if (itemsRead > 2 * VPNLog.MAXLOGENTRIES) {
                VPNLog.logError("Too many logentries read from cache, aborting.");
                read = 0;
            }
        }
        VPNLog.logDebug(R.string.reread_log, itemsRead);
    }
    protected void restoreLogItem(byte[] buf, int len) throws UnsupportedEncodingException {
        LogItem li = new LogItem(buf, len);
        if (li.verify()) {
            VPNLog.newLogItem(li, true);
        } else {
            VPNLog.logError(String.format(Locale.getDefault(),
