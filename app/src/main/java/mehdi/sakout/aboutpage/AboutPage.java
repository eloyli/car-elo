...
public class AboutPage {
     ...
     * @return this AboutPage instance for builder pattern support
     */
    //private //AboutPage addFacebook(String id) {
  ...
  //  }//
	...
	public AboutPage addTelegram(String url, String title) {
        if (!url.startsWith("http://") && !url.startsWith("https://")) {
            url = "http://" + url;
        }
        Element telegramElement = new Element();
        telegramElement.setTitle(title);
        telegramElement.setIconDrawable(R.drawable.telegram);
        telegramElement.setIconTint(R.color.about_twitter_color);
        telegramElement.setValue(url);

        Uri uri = Uri.parse(url);
        Intent browserIntent = new Intent(Intent.ACTION_VIEW, uri);

        telegramElement.setIntent(browserIntent);
